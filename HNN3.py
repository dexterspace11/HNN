...
st.write("Running 24/7 Brain-like DNN-EQIC Learning & Forecasting")
...
while True:
    ...
    st.write(f"Prediction MSE: {mse:.5f}")
    st.write(f"Prediction MAE: {mae:.5f}")
    ...
    fig = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3', title='PCA 3D Visualization (Neural Evolution)')
    st.plotly_chart(fig)

    st.write(f"Total Units Generated: {len(network.units)}")
    time.sleep(60)
