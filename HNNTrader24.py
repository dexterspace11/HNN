...
def main():
    ...
    ax_feat.plot(df.index, df['RSI'], label='RSI')
    ax_feat.plot(df.index, df['MA20'], label='MA20')
    ax_feat.plot(df.index, df['ATR'], label='ATR')
    ax_feat.legend()
    ax_feat.set_title("Technical Indicators")
    st.pyplot(fig_feat)
    ...
    # 🌈 Dream Visualization and Clustering
    st.subheader("🧠 Dream Space (PCA + Clustering)")
    ...
    if len(dream_patterns) >= 3:
        try:
            pca = PCA(n_components=2)
            reduced = pca.fit_transform(dream_patterns)
            kmeans = KMeans(n_clusters=min(5, len(dream_patterns)//2), n_init=10).fit(reduced)
            fig_dream, ax_dream = plt.subplots(figsize=(6, 4))
            scatter = ax_dream.scatter(reduced[:, 0], reduced[:, 1], c=kmeans.labels_, cmap='tab10', alpha=0.8)
            ax_dream.set_title("Clustered Dream Patterns")
            st.pyplot(fig_dream)
        except Exception as e:
            st.error(f"Dream clustering failed: {e}")
    else:
        st.info("More dream data needed for clustering.")
