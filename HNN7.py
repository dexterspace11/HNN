...
while True:
    ...
    if prev_prediction is not None and prev_timestamp != df.index[-1]:
        ...
        prediction_log.append((prev_timestamp.strftime("%H:%M:%S"), prev_prediction, actual_close, forward_error))
    ...
    with placeholder.container():
        ...
        st.metric("Prediction MSE", f"{np.mean(mse_list):.5f}")
        st.metric("Prediction MAE", f"{np.mean(mae_list):.5f}")
        st.metric("Neural Units (Centroids)", len(network.units))
        st.metric("Actual Last Close", f"{actual_close:.2f}")
        st.metric("Predicted Next Close", f"{predicted_close:.2f}")

        if prediction_log:
            log_df = pd.DataFrame(prediction_log, columns=['Time', 'Predicted', 'Actual', 'Error'])
            st.line_chart(log_df.set_index('Time')[['Predicted', 'Actual']])
