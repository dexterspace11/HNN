...
while True:
    ...
    # Derive buy/sell zones from memory units
    unit_closes = [scaler.inverse_transform([[unit.position[0], 0, 0, 0]])[0][0] for unit in network.units]
    if unit_closes:
        buy_zone = np.min(unit_closes)
        sell_zone = np.max(unit_closes)
    else:
        buy_zone = sell_zone = predicted_close
    ...
    with placeholder.container():
        ...
        col2.metric("Buy at Dip", f"{buy_zone:.2f}")
        col3.metric("Sell at Peak", f"{sell_zone:.2f}")
        ...
        ax.plot(log_df['Time'], log_df['Buy'], label='Buy Zone', linestyle='--', color='green')
        ax.plot(log_df['Time'], log_df['Sell'], label='Sell Zone', linestyle='--', color='red')
        ax.legend()
        ax.set_title("BTC/USDT Forecast & Trade Zones")
