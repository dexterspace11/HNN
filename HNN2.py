...
# === Streamlit App ===
st.title("Brain-like Hybrid DNN-EQIC BTC/USDT Predictor (Live)")
mse_list, mae_list = [], []
...
for cycle in range(1000):
    ...
    st.write(f"Units: {len(network.units)}")
    ...
    fig = px.scatter_3d(x=data_pca[:,0], y=data_pca[:,1], z=data_pca[:,2], color_discrete_sequence=['blue'])
    st.plotly_chart(fig)

    time.sleep(60)  # wait 1 min before the next cycle